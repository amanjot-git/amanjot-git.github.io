...
image: "/posts/chi-square-test-img.png"
...
Our client, a grocery retailer, launched an initiative earlier this year called the "Delivery Club." For an annual fee of $100, members receive free grocery deliveries, a service that normally costs $10 per delivery. To promote this club, the retailer conducted a campaign where customers were randomly assigned to one of three groups: one received a low-cost, low-quality mailer; another received a high-cost, high-quality mailer; and a third (control group) received no mailer.

The retailer observed that the signup rate for the Delivery Club was significantly higher among the contacted groups compared to the control group. They now seek to determine whether there was a significant difference in the effectiveness of the cheap mailer versus the expensive mailer in terms of increasing signups. Understanding this difference is critical to refining their marketing strategies and enhancing the return on investment (ROI) for future campaigns.

<br>
<br>
### Actions <a name="overview-actions"></a>

For this analysis, which is centered on comparing the sign-up rates of two groups, we utilized the Chi-Square Test for Independence. Detailed information about this test is provided in the dedicated section below.

**Note:** Another method for comparing "rates" is the Z-Test for Proportions. Although this test could have been used here, we opted for the Chi-Square Test for Independence due to several reasons:

* The test statistic generated by both tests is identical.
* The Chi-Square Test allows for data representation in 2x2 tables, which simplifies explanation to stakeholders.
* The Chi-Square Test is adaptable to scenarios involving more than two groups, offering the client a consistent method for assessing significance across different campaigns.

From the *campaign_data* table in the client's database, we focused on customers who received "Mailer 1" (low cost) and "Mailer 2" (high cost) during this campaign, and we excluded those in the control group.

We established our hypotheses and acceptance criteria for the test as follows:

**Null Hypothesis (H0):** There is no relationship between the type of mailer and the signup rate. They are independent of each other.
**Alternate Hypothesis (H1):** There is a relationship between the type of mailer and the signup rate. They are not independent of each other.
**Acceptance Criteria:** The significance level is set at 0.05.

For the Chi-Square Test for Independence, it is necessary to aggregate the data into a 2x2 matrix representing *signup_flag* by *mailer_type*. This matrix was then used as input for the algorithm in the *scipy* library to compute the Chi-Square statistic, p-value, degrees of freedom, and expected values.
